...
# -*- coding: utf-8 -*-
"""
DailyCheck Bot v4.0 - Полная модульная версия (ПОЛНЫЙ ФАЙЛ)
Telegram бот для отслеживания ежедневных привычек и задач с AI-ассистентом

Автор: AI Assistant  
Версия: 4.0.0
Дата: 2025-06-13
ПОЛНАЯ версия со ВСЕМИ функциями из оригинального файла 207KB
"""
...
class User:
    ...
    def from_dict(cls, data: dict) -> "User":
        ...
        return user # ===== СИСТЕМА ДОСТИЖЕНИЙ =====
...
- Без нумерации и дополнительных символов"""
    ...
    def _get_fallback_task_suggestions(self, category: str = None) -> List[str]:
        ...
        return random.sample(all_tasks, 5) # ===== БАЗА ДАННЫХ =====
...
class KeyboardManager:
    ...
    def get_main_keyboard() -> ReplyKeyboardMarkup:
        ...
        keyboard = [
            ...
            [KeyboardButton("⚙️ Настройки"), KeyboardButton("ℹ️ Справка")]
        ]
...
class DailyCheckBot:
    ...
    async def _register_handlers(self):
        ...
        # ⭐ КРИТИЧНО: Сначала ConversationHandler'ы (группа 0 - МАКСИМАЛЬНЫЙ приоритет)
        logger.debug("🔥 Регистрируем ConversationHandler'ы в группе 0...")
        self._register_task_creation_handlers()
        
        # 🔸 Основные команды (группа 1)
        logger.debug("📝 Регистрируем основные команды в группе 1...")
        self.application.add_handler(CommandHandler("start", self.start_command), group=1)
        self.application.add_handler(CommandHandler("help", self.help_command), group=1)
        self.application.add_handler(CommandHandler("tasks", self.tasks_command), group=1)
        self.application.add_handler(CommandHandler("add", self.add_task_start), group=1)
        self.application.add_handler(CommandHandler("stats", self.stats_command), group=1)
        self.application.add_handler(CommandHandler("achievements", self.achievements_command), group=1)
        self.application.add_handler(CommandHandler("friends", self.friends_command), group=1)
        self.application.add_handler(CommandHandler("export", self.export_command), group=1)
        self.application.add_handler(CommandHandler("settings", self.settings_command), group=1)
        
        # 🔸 Утилиты (группа 1)
        logger.debug("⚙️ Регистрируем утилиты в группе 1...")
        self.application.add_handler(CommandHandler("timer", self.timer_command), group=1)
        self.application.add_handler(CommandHandler("remind", self.remind_command), group=1)
        self.application.add_handler(CommandHandler("theme", self.theme_command), group=1)
        self.application.add_handler(CommandHandler("myid", self.myid_command), group=1)
        self.application.add_handler(CommandHandler("add_friend", self.add_friend_command), group=1)
        
        # 🔸 Быстрые команды (группа 1)
        logger.debug("⚡ Регистрируем быстрые команды в группе 1...")
        self.application.add_handler(CommandHandler("settasks", self.settasks_command), group=1)
        self.application.add_handler(CommandHandler("weekly_goals", self.weekly_goals_command), group=1)
        self.application.add_handler(CommandHandler("analytics", self.analytics_command), group=1)
        
        # 🔸 Обработчики кнопок главного меню (группа 1)
        logger.debug("🔘 Регистрируем обработчики кнопок в группе 1...")
        self.application.add_handler(MessageHandler(filters.Regex("^📝 Мои задачи$"), self.tasks_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^➕ Добавить задачу$"), self.add_task_start), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^✅ Отметить выполнение$"), self.completion_button_handler), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^📊 Статистика$"), self.stats_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^🏆 Достижения$"), self.achievements_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^👥 Друзья$"), self.friends_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^🤖 AI Чат$"), self.ai_chat_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^⏰ Таймер$"), self.timer_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^⚙️ Настройки$"), self.settings_command), group=1)
        self.application.add_handler(MessageHandler(filters.Regex("^ℹ️ Справка$"), self.help_command), group=1)
        
        # 🔸 Callback обработчики (группа 1)
        logger.debug("📞 Регистрируем callback обработчики в группе 1...")
        self.application.add_handler(CallbackQueryHandler(self.handle_callback_query), group=1)
        
        # ⚠️ AI команды (группа 2 - ПОСЛЕ всех основных функций)
        logger.debug("🤖 Регистрируем AI команды в группе 2...")
        self.application.add_handler(CommandHandler("ai_chat", self.ai_chat_command), group=2)
        self.application.add_handler(CommandHandler("motivate", self.ai_motivate_command), group=2)
        self.application.add_handler(CommandHandler("ai_coach", self.ai_coach_command), group=2)
        self.application.add_handler(CommandHandler("psy", self.ai_psychology_command), group=2)
        self.application.add_handler(CommandHandler("suggest_tasks", self.ai_suggest_tasks_command), group=2)
        
        # 🚨 AI чат (группа 3 - НИЗКИЙ приоритет!)
        logger.debug("💬 Регистрируем AI чат в группе 3...")
        self.application.add_handler(MessageHandler(
            filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE,
            self.handle_ai_chat_message
        ), group=3)
        
        # 🔒 Общий обработчик сообщений (группа 4 - ПОСЛЕДНИЙ)
        logger.debug("🔄 Регистрируем общий обработчик в группе 4...")
        self.application.add_handler(MessageHandler(filters.ALL, self.handle_unknown_message), group=4)
        ...
        logger.info(f"✅ ВСЕ {total_handlers} ОБРАБОТЧИКОВ зарегистрированы с правильными приоритетами!")
        logger.info("🎯 Приоритеты: Группа 0 (ConversationHandler) → Группа 1 (Основное) → Группа 2 (AI команды) → Группа 3 (AI чат) → Группа 4 (Общие)")
    ...
    def _register_task_creation_handlers(self):
        ...
        # 🔥 МАКСИМАЛЬНЫЙ ПРИОРИТЕТ - группа 0!
        self.application.add_handler(task_creation_handler, group=0)
        logger.info("✅ ConversationHandler для создания задач зарегистрирован (группа 0 - МАКСИМАЛЬНЫЙ ПРИОРИТЕТ)")
        ...
        self.application.add_handler(reminder_handler, group=0) # ===== ОСНОВНЫЕ КОМАНДЫ =====
...
📈 **По категориям:**"""
    ...
    # ... (все методы _handle_* из оригинального файла сохраняются здесь)
